...
public class FlutterBluetoothSerialPlugin implements MethodCallHandler, RequestPermissionsResultListener, ActivityResultListener, ViewDestroyListener {
    ...
    public void onMethodCall(MethodCall call, Result result) {
        ...
        switch (call.method) {
            ...
            case "startDiscovery":
                ensurePermissions(new EnsurePermissionsCallback() {
                    ...
                    public void onResult(boolean granted) {
                        ...
                        if (!isStarted) {
                            result.error("discovering_error", "discovering not started", null);
                            return;
                        }
            ...
            case "startDiscoverable": {
                int duration = 0;

                if (call.hasArgument("duration")) {
                    try {
                        duration = (int) call.argument("duration");
                    } catch (ClassCastException ex) {
                        result.error("invalid_argument", "'duration' argument is required to be integer", null);
                        break;
                    }
                ...
                try {
                    ...
                    setDiscoverableTimeout.invoke(bluetoothAdapter, duration);
                    setScanMode.invoke(bluetoothAdapter, BluetoothAdapter.SCAN_MODE_CONNECTABLE_DISCOVERABLE, duration);
                    result.success(null);
                } catch (Exception e) {
                    result.error("cancel_discoverable_error", "cancel discoverable failed", null);
                }
                break;
            }
